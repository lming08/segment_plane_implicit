...
const float WIN_PNT2PNT_DIST_MIN = 0.20f;  //!WindowModel类中，点到点间的距离的阈值，窗户边长的最小值
const float WIN_PNT2PNT_DIST_MAX = 2.80f;  //!WindowModel类中，点到点间的距离的阈值，窗户边长的最大值
const float WIN_PNT2LR_BILATERAL_DIST_MIN = 0.3f;  //!WindowModel类中，点到墙面左右(left right)边缘距离的最小值
const float WIN_PNT2LR_BILATERAL_DIST_MAX = 2.8f;  //!WindowModel类中，点到墙面左右(left right)边缘距离的最小值
const float WIN_PNT2UD_BILATERAL_DIST_MIN = 0.3f;  //!WindowModel类中，点到墙面上下(up down)边缘距离的最小值
const float WIN_PNT2UD_BILATERAL_DIST_MAX = 4.8f;  //!WindowModel类中，点到墙面上下(up down)边缘距离的最小值
